...
import { filmes, categoriesFilmes } from "@/data/filmes";
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="font-display text-3xl md:text-4xl font-bold text-foreground">
              Catálogo de <span className="text-gold">Filmes</span>
            </h1>
          </div>
          <p className="text-muted-foreground font-body">
            {filmes.length} filmes disponíveis na plataforma
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-10">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="font-display text-xl text-muted-foreground">Nenhum filme encontrado</p>
            <p className="text-sm text-muted-foreground mt-2 font-body">Tente mudar os filtros de busca</p>
          </div>
        ) : (
          <>
            <p className="text-sm text-muted-foreground mb-6 font-body">
              {filtered.length} filme{filtered.length !== 1 ? "s" : ""} encontrado{filtered.length !== 1 ? "s" : ""}
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {filtered.map((filme) => (
                <ContentCard key={filme.id} item={filme} />
              ))}
            </div>
          </>
        )}
